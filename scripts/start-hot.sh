...
if [[ "$KILL_PORT" == "true" ]]; then
  powershell -Command "try { if (Get-NetTCPConnection -State Listen -LocalPort 19988 -ErrorAction SilentlyContinue) { Stop-Process -Id (Get-NetTCPConnection -State Listen -LocalPort 19988 -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty OwningProcess) -Force } } catch { }"
else
  powershell -Command "try { if (Get-NetTCPConnection -State Listen -LocalPort 19988 -ErrorAction SilentlyContinue) { exit 1 } } catch { }" || {
    echo "Port 19988 already in use. Rerun with --kill-port or stop the relay process." >&2
    exit 1
  }
fi
