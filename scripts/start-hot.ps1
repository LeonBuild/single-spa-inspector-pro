...
$portInUse = Get-NetTCPConnection -State Listen -LocalPort 19988 -ErrorAction SilentlyContinue
if ($portInUse) {
  $processId = $portInUse.OwningProcess | Select-Object -First 1
  if ($KillPort) {
    Stop-Process -Id $processId -Force
    Write-Host "Killed process on port 19988: PID $processId"
  } else {
    throw "Port 19988 already in use (PID $processId). Rerun with -KillPort or stop the relay process."
  }
}
