# single-spa-inspector-pro MCP 开发指南（方案 A：CDP Relay + Playwright MCP）

本文档目标：在 **不破坏现有 single-spa-inspector-pro 的人类 DevTools 体验**（面板、覆盖、Clear Cache & Refresh）前提下，新增一套 **给 AI 使用的 MCP 服务**，使 AI 能连接到“人类正在使用的真实 Chrome Tab”，从而 **看见并操作** import-map-overrides 覆盖后的最终渲染结果。
...
- **扩展（Extension）**：`single-spa-inspector-pro` 现有 Chrome 扩展（MV3 service worker + DevTools panel + content scripts）。
...
建议在 `single-spa-inspector-pro` 下新增一个 `mcp/` 子目录（独立 Node 工程）：
...
- 默认端口：建议沿用 `19988`（与 playwriter 一致，减少理解成本）
- discovery endpoints：
  - `GET /` → `OK`
  - `GET /version` → `{ version }`
...
- **不改变 action click**：继续执行 Clear Cache & Refresh（人类路径）。
- AI Bridge 的 attach 入口不走 UI：默认后台保持与 relay 的连接即可；当 MCP 端需要控制某个 tab 时，通过 CDP 的 `Target` 流程 attach。
...
1. 新建 `single-spa-inspector-pro/mcp/package.json`
...
4. 保持现有 Clear Cache & Refresh 逻辑不动（你现在的 `browser.action.onClicked`）
...
2. 暴露 MCP tools（MVP）：
   ...
   - `ensure_fresh_render`
3. 实现 `mcp/src/cli.ts`：
   - `sspa-mcp`：启动 MCP server
   - `sspa-mcp serve`：启动 relay server（可选远程 token 模式）
...
- toolbar 点击：Clear Cache & Refresh 仍正常
